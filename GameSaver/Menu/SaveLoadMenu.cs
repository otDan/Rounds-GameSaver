...
internal class SaveLoadMenu : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        loadButton.GetComponent<Button>().onClick.AddListener(() =>
        {
            ...
            else
            {
                ...
                SelectSave(_selectedGame.gameData, _selectedSave);
    ...
    private IEnumerator LoadGameInfoDataUi(GameInfoData gameInfoData, Transform roundDisplayContainer, Transform roundButtonContainer, Transform playerContainer)
    {
        foreach (var saveData in gameInfoData.gameSaves.Where(saveData => saveData != null))
        {
            if (saveData.button == null)
            {
                ...
                roundDisplay.transform.GetChild(0).GetComponent<TextMeshProUGUI>().text =
                    $"ROUND {saveData.round}";
                roundDisplay.transform.GetChild(1).GetChild(1).GetComponent<TextMeshProUGUI>().text =
                    $"{saveData.players.Count}/32";
                roundDisplay.transform.GetChild(2).GetComponent<TextMeshProUGUI>().text =
                    gameInfoData.gameData.gameMode.ToUpper();
                ...
                {
                    GameSaver.Instance.StartCoroutine(LoadSaveDataUi(saveData, playerContainer));
    ...
    private IEnumerator LoadButtons()
    {
        ...
        foreach (var gameInfoData in orderedGames.Where(gameInfoData => gameInfoData != null))
        {
            ...
            {
                GameSaver.Instance.StartCoroutine(LoadGameInfoDataUi(gameInfoData, roundDisplayContainer, roundButtonContainer, playerContainer));
    ...
    {
        foreach (var gameInfoData in orderedGames.Where(gameInfoData => gameInfoData.gameData.button != null))
        {
            gameInfoData.gameData.button.SetActive(true);
            yield return null;
        }
    ...
    {
        foreach (var saveData in from gameInfoData in orderedGames where gameInfoData.gameData.button != null from saveData in gameInfoData.gameSaves where saveData.button != null select saveData)
        {
            saveData.button.SetActive(false);
        }
    }
    ...
    {
        foreach (var saveData in orderedGames.Where(gameInfoData => gameInfoData.gameData.button != null).SelectMany(gameInfoData => gameInfoData.gameSaves))
        {
